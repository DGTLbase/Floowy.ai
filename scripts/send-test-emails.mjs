#!/usr/bin/env node
/**
 * Send the 5 lifecycle emails (A/B/C/D/E) to a test inbox via Resend.
 * Mirrors the templates in supabase/functions/* — for previewing copy/layout.
 *
 *   RESEND_API_KEY=re_xxx node scripts/send-test-emails.mjs [email] [firstName]
 *
 * Defaults: jefcgealon@gmail.com / "Jef". Subjects are prefixed with [TEST].
 */
const KEY = process.env.RESEND_API_KEY;
if (!KEY) { console.error("✖ RESEND_API_KEY required"); process.exit(1); }
const TO = process.argv[2] || "jefcgealon@gmail.com";
const NAME = process.argv[3] || "Jef";

const FROM = "Floowy.ai <hello@floowy.ai>";
const HEADER_IMG = "https://fjzifykgvdsownlscgct.supabase.co/storage/v1/object/public/products/email-header.png";
...
const button = (t, h) => `
  <table role="presentation" cellpadding="0" cellspacing="0" style="margin:28px auto;">
    <tr><td style="border-radius:10px;background:linear-gradient(135deg,${GREEN} 0%,${GREEN_DARK} 100%);">
      <a href="${h}" style="display:inline-block;padding:15px 38px;color:#fff;text-decoration:none;font-weight:bold;font-size:16px;border-radius:10px;">${t}</a>
    </td></tr></table>`;
const statRow = (s) => `
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border-radius:12px;margin:22px 0;border:1px solid #dcfce7;"><tr>
  ${s.map((x) => `<td align="center" style="padding:14px 10px;"><div style="font-size:30px;font-weight:800;color:${GREEN_DARK};line-height:1;">${x.value}</div><div style="font-size:13px;color:${MUTED};margin-top:6px;">${x.label}</div></td>`).join("")}
  </tr></table>`;
...
<p style="margin:0;"><a href="https://floowy.ai" style="color:${GREEN_DARK};text-decoration:none;">floowy.ai</a> · <a href="${EURO1_URL}" style="color:${GREEN_DARK};text-decoration:none;">Start for €1</a> · <a href="https://floowy.ai/contact" style="color:${GREEN_DARK};text-decoration:none;">Contact</a></p>
...
const h1 = (t) => `<h1 style="font-size:25px;margin:0 0 18px;color:${INK};">${t}</h1>`;
const ul = (items) => `<ul style="margin:0 0 16px;padding-left:20px;color:${INK};">${items.map((i) => `<li style="margin-bottom:8px;">${i}</li>`).join("")}</ul>`;
const flame = (t) => `<p style="margin:0 0 10px;color:${GREEN_DARK};font-weight:bold;">⚡ ${t}</p>`;

const n = NAME;
const emails = [
  { subject: "How ICON Amsterdam cut production costs by 90%", preview: "They were spending weeks on shoots. Now they publish daily. Here's how.", press: false,
    body: h1("How ICON Amsterdam cut production costs by 90%") + p(`Hi ${n},`) +
      ...
      statRow([{ value: "+72%", label: "Production cost saved" }, { value: "+90%", label: "Faster flatlay production" }, { value: "+75%", label: "Time saved per shoot" }]) +
  ...
  { subject: "This is your last reminder", preview: "Your €1 access won't be available forever.", press: false,
    body: h1("This is your last reminder") + p(`Hi ${n},`) +
      `<p style="margin:0 0 6px;">You created a Floowy account 48 hours ago.</p>` + p("You haven't started yet.") +
      `<p style="margin:0 0 10px;">That's fine — but here's the reality:</p>` +
      ul(["Your competitors are already testing more creatives than you", "Brands using Floowy are cutting production costs by up to 90%", "The €1 offer is not permanent"]) +
      ...
      button("Activate my €1 access now", EURO1_URL) + flame("This is your last nudge from us. The offer is live — grab it.") },

  { subject: "The brands winning right now all have one thing in common", preview: "We reactivated your €1 offer. Here's why — and what they're achieving.", press: false,
    body: h1("The brands winning right now all have one thing in common") + p(`Hi ${n},`) +
      ...
      caseStudy("Marcel's Green Soap", "A Dutch sustainable cleaning brand scaling national campaigns from a single content team.", [{ value: "+36%", label: "Content output" }, { value: "-60%", label: "Production costs" }, { value: "+28%", label: "Engagement rate" }]) +
      caseStudy("Basko", "Fashion e-commerce brand solving buyer uncertainty with full-angle AI product coverage.", [{ value: "-24%", label: "Product return rate" }, { value: "+19%", label: "Conversion rate" }, { value: "+27%", label: "Add-to-cart rate" }]) +
      caseStudy("ICON Amsterdam", "Men's fashion brand replacing costly shoots with daily AI-generated on-brand visuals.", [{ value: "+72%", label: "Production costs saved" }, { value: "+90%", label: "Faster flatlay production" }, { value: "+75%", label: "Time saved per shoot" }]) +
      ...
      button("Activate my €1 access", EURO1_URL) + flame("Activate offer. This is the last time we'll reach out.") +
  ...
  { subject: "Welcome to Floowy! we're glad you're here", preview: "Thank you for the trust. Here's everything you need to get the most out of Floowy.", press: false,
    body: h1("Welcome to Floowy — we're glad you're here") + p(`Hi ${n},`) + p("Welcome to Floowy.") +
  ...
  { subject: "National television came to us. We think you should see why.", preview: "RTL7, RTLZ & Videoland visited Floowy. Here's what they found — and why it matters for you.", press: true,
    body: h1("National television came to us. We think you should see why.") + p(`Hi ${n},`) +
      ...
      statRow([{ value: "-90%", label: "Production cost — ICON Amsterdam" }, { value: "+36%", label: "Content output — Marcel's Green Soap" }, { value: "+19%", label: "Conversion rate — Basko" }]) +
      ...
      ul(["€1 to start — no risk, no big commitment", "10 credits in your first 3 days", "Your first creative ready in under 2 minutes", "Download-ready outputs — zero watermarks"]) +
      button("Watch the TV feature + start for €1", EURO1_URL) + flame("The €1 offer is live — it takes 60 seconds to start.") +
...
async function main() {
  ...
  for (const e of emails) {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: FROM, to: [TO], subject: `[TEST] ${e.subject}`, html: layout(e.body, e.preview, e.press) }),
    });
    const j = await res.json();
    console.log(res.ok ? `  ✓ ${e.subject}  (${j.id})` : `  ✖ ${e.subject}  ${JSON.stringify(j)}`);
  }
  console.log("");
}
