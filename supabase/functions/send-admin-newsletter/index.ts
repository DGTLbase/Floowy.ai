...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { subject, htmlContent, recipientEmails } = await req.json();

    if (!subject || !htmlContent || !recipientEmails || !Array.isArray(recipientEmails) || recipientEmails.length === 0) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: subject, htmlContent, recipientEmails' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Creating Brevo campaign for ${recipientEmails.length} recipients`);

    // Step 1: Create a temporary list for this campaign
    const listName = `Campaign_${Date.now()}`;
    ...
    // Step 5: Send the campaign immediately
    await brevoRequest(BREVO_API_KEY, `/emailCampaigns/${campaignId}/sendNow`, 'POST');
    console.log('Campaign sent!');

    // Record in scheduled_emails for Sent Box tracking
    await supabaseAdmin
    ...
    return new Response(
      JSON.stringify({ success: true, successCount: recipientEmails.length, failCount: 0, campaignId }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
    console.error('Error in send-admin-newsletter:', error);
    return new Response(
