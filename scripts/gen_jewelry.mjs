...
const groups = JSON.parse(fs.readFileSync('/tmp/jewelry.json','utf8'));
const slug = (s)=>s.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
fs.mkdirSync('src/assets/flatlay-refs/v10',{recursive:true});
...
for(const [subId,sub,names] of groups)
  for(const name of names)
    tasks.push({subId,sub,name,fname:`jewelry-${slug(sub)}-${slug(name)}.jpg`});

const out=[];
for(const t of tasks){
  const local=`src/assets/flatlay-refs/v10/${t.fname}`;
  let buf;
  if(fs.existsSync(local)) buf=fs.readFileSync(local);
  else {
    const prompt=`Professional e-commerce flat lay photography of a single ${t.name} (Jewelry category). The garment is laid flat on a pure white seamless background, perfectly centered, top-down camera angle, soft even studio lighting, no shadows, no model, no props, no hangers. Crisp clean product photography, neutral natural color, photorealistic, square composition.`;
...
fs.writeFileSync('/tmp/jewelry_rows.json',JSON.stringify(out,null,2));
