...
const groups = JSON.parse(fs.readFileSync('/tmp/hats.json','utf8'));
const slug = (s)=>s.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
fs.mkdirSync('src/assets/flatlay-refs/v9',{recursive:true});
...
fs.writeFileSync('/tmp/hats_rows.json',JSON.stringify(out,null,2));
